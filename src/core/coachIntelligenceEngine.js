...
export function buildCoachRecommendation(inputState = {}, now = Date.now()) {
  ...
  const cautious = confidence < 0.55 ? 'Underlaget är fortfarande begränsat, men' : confidence < 0.8 ? 'Baserat på din senaste data rekommenderar ASKR' : ''
  const headline = insufficientData ? 'ASKR behöver mer underlag.' : (cautious ? `${cautious} ${String(decision?.title || 'fortsatt planerad träning').toLowerCase()}.` : decision?.title || 'Fortsätt enligt planen')
