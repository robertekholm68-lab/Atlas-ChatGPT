import { Activity, ArrowRight, Bot, Dumbbell, HeartPulse, Home, MessageCircle, Moon, Target, TrendingUp, UserRound, Zap } from 'lucide-react'
...
function ReadinessDial({ value, hasData }) {
  ...
  if (!Number.isFinite(parsed)) return <div className="atlas-readiness-dial empty" aria-label="Readiness saknas"><strong>–</strong><span>Ej loggat</span></div>
  const score = Math.max(0, Math.min(100, Math.round(parsed)))
  return <div className="atlas-readiness-dial" style={{ '--score': `${score}%` }} aria-label={`Readiness ${score}%`}><strong>{score}</strong><span>Redo</span></div>
}
...
export function AtlasHomeScreen({ profile, core, recommendation, readiness, setPage, now }) {
  ...
  return <div className="atlas-home-page">
    ...
    <AtlasCard className="atlas-recovery-card"><div className="atlas-section-head"><span>Återhämtning</span><b>{vm.hasRecoveryData ? 'Aktuell belastning' : 'Ingen data ännu'}</b></div>{vm.muscles.length ? <div className="atlas-muscle-list">{vm.muscles.map(m => <div key={m.name}><span>{m.name}</span><AtlasProgress value={m.readiness} label={`${m.name} återhämtning`}/><b>{m.readiness}%</b></div>)}</div> : <p className="atlas-empty">Ingen lokal muskelåterhämtning loggad ännu. Logga ett pass för att visa verklig readiness.</p>}</AtlasCard>

    <AtlasCard className="atlas-coach-card"><div><span className="atlas-kicker">ASKR Coach</span><h3>{vm.coachTitle}</h3><p>{vm.coachSummary}</p></div><AtlasSecondaryButton onClick={() => go('coach')}><MessageCircle size={18}/>Fråga coachen</AtlasSecondaryButton></AtlasCard>

    <AtlasCard className="atlas-workout-access-card"><div><span className="atlas-kicker">Dagens pass</span><h3>Rekommenderat träningspass</h3><p>Behåll snabb åtkomst till dagens träningsval utan att ersätta coachkortet.</p></div><AtlasPrimaryButton onClick={() => go(vm.workoutTarget)} ariaLabel="Öppna dagens pass"><Dumbbell size={18}/>Öppna dagens pass</AtlasPrimaryButton></AtlasCard>

    <AtlasCard className="atlas-week-card"><div className="atlas-section-head"><span>Dagens statistik</span><b>{vm.weeklyCompletion !== null ? `${vm.weeklyCompletion}/${vm.weeklyTarget || '–'} pass` : 'Ingen data ännu'}</b></div>{vm.weeklyCompletion !== null ? <AtlasProgress value={vm.weeklyCompletion || 0} max={vm.weeklyTarget || 1} label="Veckans träningsmål"/> : <p className="atlas-empty">Veckostatistik visas när träningspass finns sparade.</p>}</AtlasCard>

    <AtlasCard className="atlas-activity-card"><div className="atlas-section-head"><span>Senaste aktivitet</span></div>{vm.latest ? <p><strong>{vm.latest.name || 'Träningspass'}</strong><br/><small>{vm.latest.sets ? `${vm.latest.sets} set` : 'Genomfört pass'}</small></p> : <p className="atlas-empty">Ingen data ännu. Inget genomfört pass finns sparat.</p>}</AtlasCard>
  </div>
}
