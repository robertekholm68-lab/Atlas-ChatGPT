...
import { ASKR_BUILD, ASKR_VERSION, AskrLogo, BrandedEmptyState, BrandedLoader } from '../brand.jsx'
import { authService } from './services.js'
import { friendlyError } from './errors.js'
import { installBackgroundSync, loadQueue, loadSyncStatus } from './syncEngine.js'
import './productionPlatform.css'

const settingsSections = ['ASKR', 'Version', 'Build', 'Brand information', 'Credits placeholder', 'Privacy', 'Terms placeholder', 'Account', 'Units', 'Language', 'Notifications', 'Coach', 'Theme']
...
export default function ProductionPlatform() {
  ...
  useEffect(() => {
    ...
    const uninstall = installBackgroundSync({})
    return () => { window.removeEventListener('atlas:sync-status', onSync); uninstall() }
  }, [])
  ...
  return <main className="production-platform">
    ...
    <section className="production-grid">
      <AuthCard mode={mode} setMode={setMode} submitAuth={submitAuth} message={message}/>
      <ProfileCard />
      <ArchitectureCard />
      <StorageCard uploadProgress={uploadProgress} setUploadProgress={setUploadProgress}/>
      <SettingsCard />
      <StatesCard />
    </section>
  </main>
...
function StorageCard({ uploadProgress, setUploadProgress }){ return <section className="prod-panel"><h2><Image size={20}/> Supabase Storage</h2><p>Buckets prepared for profile images, progress photos, exercise assets, future videos, coach attachments and avatars.</p><button className="secondary-prod" onClick={() => { setUploadProgress(0); const id = setInterval(() => setUploadProgress(v => v >= 100 ? (clearInterval(id), 100) : v + 20), 120) }}><Upload size={16}/> Simulate upload</button><div className="upload-track"><span style={{ width: `${uploadProgress}%` }}/></div></section> }
function SettingsCard(){ const details = { ASKR: 'Official brand identity', Version: ASKR_VERSION, Build: ASKR_BUILD, 'Brand information': 'Strength · Precision · Recovery', 'Credits placeholder': 'Open-source acknowledgements', Privacy: 'Privacy policy placeholder', 'Terms placeholder': 'Terms placeholder' }; return <section className="prod-panel settings-panel"><h2><Smartphone size={20}/> Settings</h2>{settingsSections.map(section => <button key={section}>{section}<span>{details[section] || 'Configure'}</span></button>)}</section> }
function StatesCard(){ return <section className="prod-panel"><h2><CheckCircle2 size={20}/> UX states</h2><BrandedLoader label="Syncing ASKR"/><BrandedEmptyState title="No notifications yet"><span>ASKR will show concise training, recovery and account updates here.</span></BrandedEmptyState><div className="success-state"><AskrLogo size="xs" showWordmark={false}/> Profile saved</div><div className="error-state">Retry available when network returns</div></section> }
