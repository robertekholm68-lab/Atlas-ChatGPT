...
import {
  Activity, Apple, Archive, ArrowDown, ArrowUp, BarChart3, BatteryCharging, Bot, CalendarDays, Check,
  AlertCircle,
...
export default function AppPhase4(){
  ...
  return <div className="p4-shell">
    ...
    <main className="p4-main"><header className="p4-top askr-dashboard-header"><div><span className="eyebrow">{new Date().toLocaleDateString('sv-SE', { weekday: 'long', day: 'numeric', month: 'long' })}</span><h1>{titleFor(page)}</h1><p>{subtitleFor(page)}</p></div><div><button className="p4-icon" aria-label="Dela eller exportera vy" onClick={()=>setModal('share')}><Share2 size={19}/></button><button className="p4-primary" onClick={()=>setModal('new-program')}><Plus size={18}/>Nytt program</button></div></header>
...
function Dashboard({programs,history,startProgram,setPage}){
  ...
  return <div className="program-dashboard"><section className="p4-hero span8 dashboard-recovery-card" aria-labelledby="dashboard-recovery-title"><div><span className="pill"><HeartPulse size={15}/>Recovery först</span><h2 id="dashboard-recovery-title"><span>82</span><small>% readiness</small></h2><p>Stabil återhämtning för ett kvalitativt styrkepass. Håll 1–2 repetitioner i reserv på de tyngsta seten.</p><div className="recovery-support-grid"><span><Moon size={14}/><b>7 h 24 m</b><small>sömn</small></span><span><HeartPulse size={14}/><b>52 bpm</b><small>vilopuls</small></span><span><BatteryCharging size={14}/><b>Low</b><small>stress</small></span><span><Target size={14}/><b>{meta.phase}</b><small>programfas</small></span></div><div className="hero-actions"><button className="p4-primary" onClick={()=>startProgram(active)}><Play size={18}/>Starta nästa workout</button><button className="p4-secondary" onClick={()=>setPage('recovery')}>Visa recovery</button></div></div><div className="readiness readiness-hero" aria-label="Readiness 82 procent"><strong>82</strong><span>Redo</span></div></section>
    <section className="panel span4 dashboard-coach-card"><SectionTitle eyebrow="AI Coach" title="Dagens rekommendation"/><h3>{active?.name||'Välj program först'}</h3><p>{workoutIntelligence.notes}</p><div className="coach-reasoning"><span><HeartPulse size={14}/>82% readiness</span><span><Clock size={14}/>{meta.sessionLength} min plan</span></div><button className="p4-primary full" onClick={()=>startProgram(active)}><Play size={17}/>Starta rekommendation</button><button className="p4-secondary full" onClick={()=>setPage('coach')}>Öppna AI Coach</button></section>
    <section className="panel span12 dashboard-statistics" aria-labelledby="today-statistics-title"><SectionTitle eyebrow="I dag" title="Today’s statistics"/><div className="dashboard-stat-grid"><article className="primary-stat"><span>Veckoadherence</span><strong>{adherence}%</strong><small>{completedThisWeek}/{active?.days||0} pass denna vecka</small><div className="progress-track"><i style={{width:`${adherence}%`}}/></div></article><article><CalendarDays size={17}/><span>Planerade pass</span><strong>{totalSessions}</strong></article><article><Check size={17}/><span>Genomförda</span><strong>{meta.completed}</strong></article><article><Target size={17}/><span>Arbetsset</span><strong>{meta.sets}</strong></article><article><Trophy size={17}/><span>Fas</span><strong>{meta.phase}</strong></article></div></section>
