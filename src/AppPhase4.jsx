...
import { buildGoalProfile, buildWorkoutCompletionFeedback, explainCoachDecision, makeCoachDecision } from './engines/index.js'
...
function HistoryView({history,completedWorkout}){
  ...
  return <div className="completion-layout">{completedWorkout&&<section className="panel completion-celebration workout-feedback"><span className="pill"><Check size={15}/>Workout complete</span><h2>Passet är sparat.</h2><p>{feedback?.coachComment || `${completedWorkout.name} används nu i din träningshistorik.`}</p><div className="completion-stats"><div><strong>{(feedback?.musclesImproved || []).length}</strong><span>Muscles improved</span></div><div><strong>{feedback?.recoveryImpact || '—'}</strong><span>Recovery impact</span></div><div><strong>{(feedback?.volumeAdded || completedWorkout.volume).toLocaleString('sv-SE')} kg</strong><span>Volume added</span></div><div><strong>{feedback?.newPr?.length || 0}</strong><span>New PR</span></div><div><strong>{completedWorkout.duration} min</strong><span>Duration</span></div><div><strong>{completedWorkout.sets}</strong><span>Total sets</span></div></div></section>}<section className="panel"><div className="history-toolbar"><SectionTitle eyebrow="Alla pass" title="Träningshistorik"/><div className="search-box small"><Search size={17}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Filtrera historik" aria-label="Filtrera träningshistorik"/></div></div><div className="history-table"><div className="history-row head"><span>Datum</span><span>Pass</span><span>Gym</span><span>Set</span><span>Volym</span><span>Tid</span></div>{filtered.map(h=><div className="history-row" key={h.id}><span>{h.date}</span><strong>{h.name}</strong><span>{h.gym}</span><span>{h.sets}</span><span>{h.volume.toLocaleString('sv-SE')} kg</span><span>{h.duration} min</span></div>)}</div></section></div>
}
