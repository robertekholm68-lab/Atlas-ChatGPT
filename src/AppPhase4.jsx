...
export default function AppPhase4(){
  ...
  return <div className="p4-shell">
    ...
    <main className="p4-main"><header className="p4-top"><div><span className="eyebrow">Fas 4</span><h1>{titleFor(page)}</h1><p>{subtitleFor(page)}</p></div><div><button className="p4-icon" onClick={()=>setModal('share')}><Share2 size={19}/></button><button className="p4-primary" onClick={()=>setModal('new-program')}><Plus size={18}/>Nytt program</button></div></header>
      {page==='dashboard'&&<Dashboard programs={programs} history={history} activeProgramId={activeProgramId} startProgram={startProgram} setPage={setPage}/>}
      {page==='programs'&&<ProgramLibrary programs={programs} history={history} setPrograms={setPrograms} activeProgramId={activeProgramId} setActiveProgramId={setActiveProgramId} startProgram={startProgram} notify={notify}/>}
...
function Dashboard({programs,history,activeProgramId,startProgram,setPage}){
  ...
  const recommended=programs.find(program=>program.id===activeProgramId)||programs.find(program=>program.id==='upper-a')||programs[0]
  ...
  return <div className="p4-grid workout-command-center"><section className="p4-hero span8 workout-home-hero"><div><span className="pill"><Sparkles size={15}/>Aktivt program</span><h2>{recommended?.name||'Välj workout'}</h2><p>{workoutIntelligence.notes} Fokus: {muscleFocus || 'helkropp'}.</p><div className="overview-meta"><span><Clock size={14}/>55 min</span><span><Dumbbell size={14}/>{nextExercises.length} övningar</span><span><Target size={14}/>{sets} set denna vecka</span></div><div className="hero-actions"><button className="p4-primary" onClick={()=>startProgram(recommended)}><Play size={18}/>Starta dagens pass</button><button className="p4-secondary" onClick={()=>setPage('programs')}>Visa programmet</button></div></div><div className="hero-asset-stack"><AtlasAsset src={recommended?.cover || atlasAssets.programCovers.upperA} alt={`${recommended?.name || 'Dagens workout'} program cover`} ratio="4 / 3" fallback={assetFallbackLabels.program}/><div className="readiness"><strong>84</strong><span>Redo</span></div></div></section>
...
function ProgramLibrary({programs,history,setPrograms,activeProgramId,setActiveProgramId,startProgram,notify}){
  const [filter,setFilter]=useState('alla');const [query,setQuery]=useState('');const [sort,setSort]=useState('active');const active=programs.find(p=>p.id===activeProgramId)||programs[0];const historyProgress=name=>history.filter(h=>h.name===name).length
  const programMeta=p=>{const exercises=p.exercises.map(id=>exerciseBank.find(e=>e.id===id)).filter(Boolean);const sets=exercises.reduce((sum,e)=>sum+Number((e.sets.match(/\d+/)||[3])[0]),0);const muscles=[...new Set(exercises.map(e=>e.muscle))].slice(0,3).join(' · ');return {sets,muscles,minutes:Math.max(35,exercises.length*8+8),phase:p.favorite?'Prioriterad':'Bas'}}
  const visible=programs.filter(p=>filter==='arkiv'?p.archived:!p.archived&&(filter==='favorit'?p.favorite:true)).filter(p=>(p.name+p.type+programMeta(p).muscles).toLowerCase().includes(query.toLowerCase())).sort((a,b)=>sort==='name'?a.name.localeCompare(b.name,'sv'):sort==='days'?b.days-a.days:Number(b.id===activeProgramId)-Number(a.id===activeProgramId))
  ...
  return <div className="program-layout"><section className="panel program-list"><div className="toolbar program-toolbar"><div className="segmented">{[['alla','Aktiva'],['favorit','Favoriter'],['arkiv','Arkiv']].map(([id,l])=><button key={id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{l}</button>)}</div><label className="search-box small"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Sök program"/></label><select aria-label="Sortera program" value={sort} onChange={e=>setSort(e.target.value)}><option value="active">Aktivt först</option><option value="name">Namn</option><option value="days">Dagar/vecka</option></select></div><div className="program-cards">{visible.map(p=>{const meta=programMeta(p);return <button key={p.id} className={activeProgramId===p.id?'selected':''} onClick={()=>setActiveProgramId(p.id)} aria-label={`Välj ${p.name}`}><span className="program-card-figure"><AtlasAsset src={p.muscleFigure || atlasAssets.muscleFigures.front} alt={p.muscleFigureAlt || `${p.name} målmuskelkarta`} ratio="3 / 4" fit="contain" fallback={assetFallbackLabels.muscle}/></span><span><strong>{p.name}</strong><small>{p.type} · {meta.minutes} min · {meta.sets} set</small><small>{meta.muscles}</small></span>{activeProgramId===p.id&&<span className="active-program-badge">Aktivt</span>}{p.favorite&&<Star size={16} fill="currentColor"/>}<ChevronRight size={17}/></button>})}</div></section>
    {active&&<section className="panel program-editor"><div className="editor-head"><div><span className="eyebrow">Programeditor</span><input value={active.name} onChange={e=>update(active.id,{name:e.target.value})}/><p>{active.type} · {active.days} dagar/vecka</p></div><div><button className="p4-icon" onClick={()=>update(active.id,{favorite:!active.favorite})}><Star size={18} fill={active.favorite?'currentColor':'none'}/></button><button className="p4-icon" onClick={()=>update(active.id,{archived:!active.archived})}><Archive size={18}/></button><button className="p4-primary" onClick={()=>startProgram(active)}><Play size={17}/>Starta</button></div></div>
      <div className="program-progress"><ProgramProgress value={Math.round((historyProgress(active.name)/Math.max(1,active.days))*100)} label="Veckans progress"/><div><strong>{programMeta(active).phase}</strong><span>{programMeta(active).muscles || 'Helkropp'} · nästa pass redo</span></div></div>
...
function ExerciseLibrary({notify}){
  ...
  return <div className="exercise-layout"><section className="panel exercise-browser"><div className="search-box"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Sök övning, muskel eller maskin"/></div><div className="filter-selects"><select value={muscle} onChange={e=>setMuscle(e.target.value)}>{['Alla',...new Set(exerciseBank.map(e=>e.muscle))].map(x=><option key={x}>{x}</option>)}</select><select value={equipment} onChange={e=>setEquipment(e.target.value)}>{['Alla',...new Set(exerciseBank.map(e=>e.equipment))].map(x=><option key={x}>{x}</option>)}</select></div><div className="exercise-results">{filtered.map(e=><button key={e.id} className={selected.id===e.id?'selected':''} onClick={()=>setSelected(e)}><span className="score">{e.score}</span><span><strong>{e.name}</strong><small>{e.muscle} · {e.equipment}</small></span><ChevronRight size={17}/></button>)}</div></section>
    <section className="panel exercise-detail"><div className="exercise-visual"><AtlasAsset src={selected.image} alt={`${selected.name} demonstration`} ratio="16 / 9" fit="cover" fallback={assetFallbackLabels.exercise}/><span>START</span><span>SLUT</span></div><span className="pill">Exercise Score {selected.score}%</span><h2>{selected.name}</h2><p>{selected.muscle} · sekundärt: {selected.secondary}</p><div className="detail-grid"><div><span>Utrustning</span><strong>{selected.equipment}</strong></div><div><span>Nivå</span><strong>{selected.level}</strong></div><div><span>Standard</span><strong>{selected.sets}</strong></div><div><span>Finns på</span><strong>{selected.gym}</strong></div></div><div className="instruction"><h3>Utförande</h3><ol><li>Ställ in en stabil startposition.</li><li>Kontrollera den excentriska fasen.</li><li>Arbeta genom full, smärtfri rörelse.</li></ol><h3>Vanligt fel</h3><p>För hög belastning som förkortar rörelsen och flyttar arbetet från målmuskel.</p></div><button className="p4-primary full" onClick={()=>notify('Övningen tillagd i favoriter')}><Star size={17}/>Spara övning</button></section>
  </div>
}
...
function ProgramProgress({value,label}){const safe=Math.min(100,Math.max(0,value||0));return <div className="program-progress-ring" aria-label={`${label} ${safe}%`}><span style={{background:`conic-gradient(var(--atlas-lime) ${safe}%, rgba(255,255,255,.08) 0)`}}><b>{safe}%</b></span><small>{label}</small></div>}
