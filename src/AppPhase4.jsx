...
import { createRestTimer, extendRestTimer, formatRestTime, normalizeRestTimer, pauseRestTimer, remainingRestSeconds, resumeRestTimer, skipRestTimer } from './restTimerEngine'
import {
...
export default function AppPhase4(){
  ...
  const startProgram=program=>{
    const exercises=program.exercises.map(id=>{
      ...
      return {...ex,restDurationSeconds:90,sets:Array.from({length:Number(setCount)},(_,i)=>({id:`${id}-${i}`,kg:previousKg(id),reps:8,rpe:8,tempo:'3-1-1',rest:90,done:false}))}
    })
...
function LiveSession({session,setSession,finishSession}){
  ...
  const setRestTimer=updater=>setSession(s=>({...s,restTimer:updater(normalizeRestTimer(s.restTimer,Date.now()))}))
  const toggleDone=(i,set)=>setSession(s=>({...s,exercises:s.exercises.map((e,ei)=>ei!==activeIndex?e:{...e,sets:e.sets.map((item,index)=>index===i?{...item,done:!set.done}:item)}),restTimer:!set.done&&autoRest?createRestTimer(effectiveRestDuration(active,set),Date.now()):s.restTimer}))
