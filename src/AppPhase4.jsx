import './phase4.css'
import { ActionButton, BottomNavigation, Card, EmptyState, ExerciseRow, ProgressRing, SectionTitle as AtlasSectionTitle, StatCard, WorkoutCard } from './atlasDesignSystem'
import { useEffect, useMemo, useRef, useState } from 'react'
import { getAtlasState } from './core/atlasStore'
import { formatRestTime, normalizeRestTimer, remainingRestSeconds } from './restTimerEngine'
...
import { buildGoalProfile, buildWorkoutCompletionFeedback, explainCoachDecision, getProgressIntelligence, makeCoachDecision } from './engines/index.js'
import BodyIntelligenceDashboard from './BodyIntelligenceDashboard.jsx'
import VoiceLogControl from './voice/VoiceLogControl.jsx'
import {
...
const intelligenceExerciseIds = { bench:'bench-press', row:'seated-row', ohp:'shoulder-press', pulldown:'lat-pulldown', legpress:'leg-press', curl:'incline-curl' }
const exerciseBank = exerciseBankBase.map(exercise => {
  const intelligence = intelligenceExerciseLibrary.find(item => item.id === intelligenceExerciseIds[exercise.id])
  return {
    ...exercise,
    restDurationSeconds: intelligence?.restDurationSeconds || 90,
    exerciseDna: intelligence?.exerciseDna || { movementPattern: exercise.type?.toLowerCase() || 'other', fatigueFactor: 1, hypertrophyRating: 3, strengthRating: 3, skillRating: 2, equipmentCategory: exercise.mode?.toLowerCase() || 'other', laterality: 'bilateral' },
    activationWeights: intelligence?.activationWeights || { primary: {}, secondary: {} },
    primaryMuscles: intelligence?.primary || [],
    secondaryMuscles: intelligence?.secondary || []
  }
})
...
const workoutIntelligence = {
  recoveryStatus: 'Not enough recovery data yet',
...
export default function AppPhase4(){
  ...
  const startProgram=program=>{
    const previousValues=Object.fromEntries(program.exercises.map(id=>[id,{kg:previousKg(id),reps:8}]))
    setSession(startWorkout(program,exerciseBank,{mode:'real',previousValues}))
  ...
  const finishSession=()=>{
    ...
    const feedback=buildWorkoutCompletionFeedback([...history,summary],exerciseBank,getAtlasState().recovery)
    const completed={...summary,gym:'Nordic Wellness',prs:feedback.newPr.length,calories:Math.round(sets.length*18+summary.volume/95),recovery:sets.length>14?'36–42 timmar':'24–30 timmar',feedback}
    setHistory(h=>saveCompletedWorkout(h,completed))
  ...
  return <div className="p4-shell">
    ...
    <main className="p4-main"><header className="p4-top"><div><span className="eyebrow">Fas 4</span><h1>{titleFor(page)}</h1><p>{subtitleFor(page)}</p></div><div><button className="p4-icon" onClick={()=>setModal('share')}><Share2 size={19}/></button><button className="p4-primary" onClick={()=>setModal('new-program')}><Plus size={18}/>Nytt program</button></div></header>
      ...
      {page==='exercises'&&<ExerciseLibrary notify={notify} history={history}/>}
      {page==='calendar'&&<CalendarView history={history}/>}
      {page==='history'&&<HistoryView history={history} completedWorkout={completedWorkout} onDismiss={()=>setCompletedWorkout(null)}/>}
      {page==='stats'&&<StatsView history={history}/>}
      {page==='food'&&<FoodView notify={notify}/>}
      {page==='progress'&&<ProgressView history={history} completedWorkout={completedWorkout}/>}
      ...
      {page==='session'&&session&&<LiveSession session={session} setSession={setSession} finishSession={finishSession} notify={notify}/>}
...
function Dashboard({programs,history,startProgram,setPage}){
  ...
  const goalProfile=buildGoalProfile({goal:'general_fitness',experienceLevel:'intermediate',trainingDays:active?.days,availableTime:meta.sessionLength,preferredSplit:active?.type==='Upper/Lower'?'upper':'full_body'})
  const savedRecovery=getAtlasState().recovery
  const coachDecision=makeCoachDecision({workoutHistory:history.map(item=>({...item,recommendation:item.name?.includes('Under')?'lower':'upper'})),goalProfile,recovery:savedRecovery?.updatedAt?savedRecovery:{}})
  const explanation=explainCoachDecision(coachDecision,'sv')
  return <div className="program-dashboard"><section className="p4-hero span8 program-hero"><div><span className="pill"><Sparkles size={15}/>Aktivt program</span><h2>{active?.name||'Inget aktivt program'}</h2><p>{active?`${meta.goal}. Vecka 3 av ${meta.duration} · fas ${meta.phase}. Nästa steg är tydligt: starta rekommenderat pass när du är redo.`:'Välj ett program i biblioteket för att få en tydlig plan.'}</p><div className="overview-meta"><span><CalendarDays size={14}/>Vecka 3 / {meta.duration}</span><span><Dumbbell size={14}/>{active?.days||0} dagar/vecka</span><span><Clock size={14}/>{meta.sessionLength} min/pass</span><span><Target size={14}/>{meta.muscles.slice(0,3).join(' · ')}</span></div><div className="hero-actions"><button className="p4-secondary" onClick={()=>setPage('programs')}>Visa programdetaljer</button></div></div><div className="program-orbit"><strong>{adherence}%</strong><span>adherence</span></div></section>
    <section className="panel span4 next-action-panel"><SectionTitle eyebrow="Nästa workout" title="Redo att starta"/><p>{workoutIntelligence.notes}</p><button className="p4-secondary full" onClick={()=>setPage('programs')}>Byt eller redigera</button></section>
    <section className="panel span12 askr-coach-card"><div className="askr-coach-heading"><div><span className="eyebrow">ASKR Coach · Dagens rekommendation</span><h3>{explanation.headline} · {active?.name}</h3></div><strong aria-label={`Säkerhet ${coachDecision.confidence} procent`}>{coachDecision.confidence}%<small>Säkerhet</small></strong></div><p>{explanation.summary}</p><div className="askr-coach-actions"><button className="p4-primary" onClick={()=>startProgram(active)}><Play size={18}/>Starta föreslaget pass</button><details><summary>Varför?</summary><div>{explanation.supportingPoints.map(point=><p key={point}>{point}</p>)}<p>{explanation.dataQualityMessage}</p></div></details></div></section>
...
function ExerciseLibrary({notify,history=[]}){
  ...
  const progress=useMemo(()=>getProgressIntelligence(history,null,exerciseBank).records.personalRecords.find(record=>record.exerciseId===selected.id),[history,selected.id])
  return <div className="exercise-layout premium-browser"><section className="panel exercise-browser"><SectionTitle eyebrow="Exercise Browser" title="Snabb övningssök"/><div className="search-box fast-search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Sök övning, muskel, utrustning…" autoComplete="off"/></div><div className="category-chips">{categories.map(id=><button key={id} className={quick===id?'active':''} onClick={()=>setQuick(id)}>{id==='all'?'Alla':id==='favorites'?'Favoriter':id==='recent'?'Nyligen': 'Custom'}</button>)}</div><div className="filter-selects"><select value={muscle} onChange={e=>setMuscle(e.target.value)}>{muscles.map(x=><option key={x}>{x}</option>)}</select><select value={equipment} onChange={e=>setEquipment(e.target.value)}>{equipmentOptions.map(x=><option key={x}>{x}</option>)}</select><select value={type} onChange={e=>setType(e.target.value)}>{['Alla','Compound','Isolation'].map(x=><option key={x}>{x}</option>)}</select></div><div className="exercise-results large-cards">{filtered.map(e=><button key={e.id} className={selected.id===e.id?'selected':''} onClick={()=>setSelected(e)}><span className="score">{e.score}</span><span><strong>{e.name}</strong><small>{e.muscle} · {e.equipment} · {e.type||'Compound'}</small></span>{(favorite[e.id]||e.favorite)&&<Star size={16} fill="currentColor"/>}<ChevronRight size={17}/></button>)}</div></section>
    <section className="panel exercise-detail premium-detail"><div className="exercise-visual"><AtlasAsset src={selected.image} alt={`${selected.name} hero`} ratio="16 / 8" fit="cover" fallback={assetFallbackLabels.exercise}/><span>STARTPOSITION</span><span>SLUTPOSITION</span></div><div className="detail-headline"><span className="pill">Exercise Score {selected.score}%</span><button className="p4-icon" aria-label="Favorit" onClick={()=>setFavorite(f=>({...f,[selected.id]:!f[selected.id]}))}><Star size={18} fill={(favorite[selected.id]||selected.favorite)?'currentColor':'none'}/></button></div><h2>{selected.name}</h2><p>{selected.muscle} · sekundärt: {secondary.join(' · ')||'ingen'} · recovery: not enough data yet</p><div className="detail-grid"><div><span>Primary muscles</span><strong>{selected.muscle}</strong></div><div><span>Secondary muscles</span><strong>{secondary.join(' · ')||'—'}</strong></div><div><span>Difficulty</span><strong>{selected.level}</strong></div><div><span>Equipment</span><strong>{selected.equipment}</strong></div><div><span>Exercise type</span><strong>{selected.type||'Compound'}</strong></div><div><span>Training load</span><strong>{selected.score>94?'Hög effekt':'Kontrollerad'}</strong></div></div><section className="exercise-progress" aria-label="Exercise progress"><h3>Progress</h3><div className="detail-grid"><div><span>Best weight</span><strong>{progress?`${progress.bestWeight} kg`:'—'}</strong></div><div><span>Last weight</span><strong>{progress?`${progress.lastWeight} kg`:'—'}</strong></div><div><span>Estimated 1RM</span><strong>{progress?`${progress.bestEstimated1RM} kg`:'—'}</strong></div><div><span>Frequency</span><strong>{progress?`${progress.frequency} pass`:'—'}</strong></div><div><span>Favorite rep range</span><strong>{progress?.favoriteRepRange||'—'}</strong></div><div><span>Favorite equipment</span><strong>{progress?.favoriteEquipment||selected.equipment}</strong></div></div><p className="coach-note">Coach note · {progress?'Behåll kontrollerad progression från din senaste registrerade belastning.':'Logga ett genomfört set för en personlig notering.'}</p></section><MuscleVolume compact live={muscleContribution[selected.id]||{[selected.muscle]:1}}/><div className="instruction"><h3>Instruktioner</h3><ol><li>Bygg en stabil startposition och kontrollera andningen.</li><li>Flytta vikten genom full, smärtfri rörelse med jämnt tempo.</li><li>Avsluta setet när tekniken bryts eller planerad RPE uppnås.</li></ol><h3>Common mistakes</h3><p>För hög belastning, kort rörelsebana och tappad bålspänning som flyttar arbetet från målmuskel.</p><h3>Tips</h3><p>{techniqueTip(selected.id)}</p></div><button className="p4-primary full" onClick={()=>notify('Övningen tillagd i favoriter')}><Star size={17}/>Spara övning</button></section>
  </div>
}

function LiveSession({session,setSession,finishSession,notify}){
  const [activeIndex,setActiveIndex]=useState(0)
  const [autoRest,setAutoRest]=useState(true)
  const [now,setNow]=useState(Date.now())
  const completionFeedbackRef=useRef(false)
  ...
  const restTimer=normalizeRestTimer(session.restTimer,now)
  const restRemaining=remainingRestSeconds(restTimer,now)
  useEffect(()=>{const id=setInterval(()=>setNow(Date.now()),1000);const sync=()=>setNow(Date.now());document.addEventListener?.('visibilitychange',sync);return()=>{clearInterval(id);document.removeEventListener?.('visibilitychange',sync)}},[])
  useEffect(()=>{if(session.restTimer&&restTimer?.status!==session.restTimer.status){setSession(s=>({...s,restTimer}))}},[restTimer?.status])
  useEffect(()=>{if(restTimer?.status==='completed'&&!completionFeedbackRef.current){completionFeedbackRef.current=true;if(typeof navigator!=='undefined'&&navigator.vibrate)navigator.vibrate(180)}else if(restTimer?.status==='running'){completionFeedbackRef.current=false}},[restTimer?.status])
  const updateSet=(si,patch)=>setSession(s=>updateWorkoutSet(s,activeIndex,si,patch))
  const addSet=()=>setSession(s=>addWorkoutSet(s,activeIndex,Date.now()))
  const deleteSet=si=>setSession(s=>removeWorkoutSet(s,activeIndex,si))
  ...
  const toggleDone=i=>setSession(s=>completeWorkoutSet(s,activeIndex,i,Date.now(),autoRest))
  const voiceSetIndex=Math.max(0,active.sets.findIndex(set=>!set.done))
  const voiceSet=active.sets[voiceSetIndex]
  const voiceContext={exerciseId:active.id,exerciseName:active.name,currentSetIndex:voiceSetIndex,plannedWeightKg:voiceSet?.kg,plannedReps:voiceSet?.reps,previousSet:active.sets[voiceSetIndex-1]||null,previousPerformance:previousPerformance(active.id),availableSetCount:active.sets.length}
...
function WorkoutLanding({programs,startProgram}){const today=programs.find(p=>p.id==='upper-a')||programs[0];const exercises=today.exercises.map(id=>exerciseBank.find(e=>e.id===id)).filter(Boolean);const targetMuscles=[...new Set(exercises.map(e=>e.muscle))].slice(0,4).join(' · ');const estimatedSets=exercises.reduce((sum,e)=>sum+Number((e.sets.match(/\d+/)||[3])[0]),0);return <div className="p4-grid workout-overview"><Card className="span8 atlas-hero-mobile workout-overview-hero"><span className="pill"><Dumbbell size={15}/>Dagens workout</span><h2>{today.name}</h2><p>En fokuserad pre-workout dashboard med allt beslutstöd samlat före första setet.</p><div className="overview-meta premium-dashboard-meta"><span><Clock size={14}/>55 min</span><span><Flame size={14}/>{workoutIntelligence.difficulty}</span><span><Target size={14}/>{targetMuscles}</span><span><HeartPulse size={14}/>Not enough recovery data yet</span></div><button className="p4-primary start-workout-large" onClick={()=>startProgram(today)}><Play size={19}/>START</button></Card><StatCard icon={Clock} label="Estimated duration" value="55 min" note="inkl. vila"/><StatCard icon={Flame} label="Difficulty" value={workoutIntelligence.difficulty} note="RPE 7–8"/><StatCard icon={Target} label="Expected volume" value={`${workoutIntelligence.expectedVolume.toLocaleString('sv-SE')} kg`} note={`${estimatedSets} arbetsset`}/><StatCard icon={HeartPulse} label="Recovery status" value="—" note="Not enough recovery data yet"/><section className="panel span5"><SectionTitle eyebrow="Senaste pass" title="Previous workout summary"/><div className="summary-callout"><History size={20}/><strong>{workoutIntelligence.previousSummary}</strong><span>Repetera vinnande ordning och höj bara där kvaliteten känns hög.</span></div></section><section className="panel span7"><SectionTitle eyebrow="Dagens pass" title="Target muscles & övningar"/><div className="atlas-card-stack compact">{exercises.slice(0,5).map((e,i)=><div className="landing-exercise" key={e.id}><b>{i+1}</b><span><strong>{e.name}</strong><small>{e.muscle} · {e.equipment} · {e.sets}</small></span><ChevronRight size={17}/></div>)}</div></section></div>}
...
function hasRecoveryData(recovery) {
  return Boolean(recovery?.updatedAt || Object.keys(recovery?.muscles || {}).length)
}

function RecoveryView(){
  const recovery=getAtlasState().recovery
  const hasData=hasRecoveryData(recovery)
  const muscles=Object.entries(recovery?.muscles || {}).map(([muscle,value])=>({muscle,readiness:Math.max(0,Math.round(100-Number(value?.fatigue||0)))}))
  return <div className="p4-grid recovery-page"><Card className="span8 atlas-hero-mobile recovery-glow recovery-hero"><span className="pill"><Moon size={15}/>Recovery command center</span><h2>{hasData?`Återhämtning ${recovery.score}%`:'Not enough recovery data yet'}</h2><p>{hasData?'ASKR visar återhämtning från sparad tränings- och återhämtningsdata.':'Logga pass eller öppna Recovery för att bygga en personlig återhämtningsbild.'}</p><div className="recovery-pills"><span>Uses saved app data</span><span>No simulated health metrics</span></div></Card><Card className="span4 center-card recovery-score-card">{hasData?<><ProgressRing value={recovery.score} label="ready"/><strong>Recovery data available</strong><span>Use saved readiness to guide your next session.</span></>:<EmptyState icon={HeartPulse} title="Not enough recovery data yet" action="Logga pass"><span>Complete a workout to start building recovery signals.</span></EmptyState>}</Card><StatCard icon={Moon} label="Sleep" value="—" note="Not enough recovery data yet"/><StatCard icon={HeartPulse} label="Resting heart rate" value="—" note="No saved heart-rate data"/><StatCard icon={Activity} label="Training load" value={hasData?`${recovery.score}%`:'—'} note={hasData?'Saved recovery score':'Not enough recovery data yet'}/><StatCard icon={Waves} label="Stress" value="—" note="No saved stress data"/><section className="panel span7 recovery-plan"><SectionTitle eyebrow="Next step" title="Recovery forecast"/>{hasData?<p><Sparkles size={16}/>ASKR uses saved recovery data only; no health data is simulated.</p>:<p><Sparkles size={16}/>Not enough recovery data yet. Complete workouts or add recovery entries before forecast details appear.</p>}</section><section className="panel span5 recovery-plan"><SectionTitle eyebrow="Muscle readiness" title="Local load map"/>{muscles.length?<div className="recovery-muscle-list">{muscles.map(({muscle,readiness})=><div key={muscle}><span><strong>{muscle}</strong><small>Saved app data</small></span><b>{readiness}%</b></div>)}</div>:<p className="atlas-empty">Not enough recovery data yet.</p>}</section></div>
}

function CoachView({notify}){return <div className="coach-premium"><div className="coach-orb"><Bot size={42}/></div><span className="pill"><Sparkles size={15}/>Coach</span><h2>Vad vill du optimera idag?</h2><p>ASKR Coach återanvänder befintliga notifieringar och introducerar ingen ny AI-motor.</p><div className="prompt-grid">{['Hur bör jag träna idag?','Vad säger min återhämtning?','Justera veckans plan'].map(q=><button key={q} onClick={()=>notify('Coach-fråga vald')}><Sparkles size={18}/><span>{q}</span><ChevronRight size={18}/></button>)}</div></div>}
...
function HistoryView({history,completedWorkout,onDismiss}){
  const [q,setQ]=useState('')
  const filtered=history.filter(h=>(`${h.name || ''}${h.gym || ''}${h.date || ''}`).toLowerCase().includes(q.toLowerCase()))
  const feedback=completedWorkout?.feedback
  return <div className="completion-layout">{completedWorkout&&<section className="panel workout-feedback compact-feedback" aria-label="Workout completion summary"><div><span className="pill"><Check size={15}/>Workout complete</span><h2>Passet är sparat.</h2></div><ul>{feedback?.newPr?.[0]&&<li><Check size={17}/>Nytt {feedback.newPr[0].exerciseName} PR</li>}<li><Check size={17}/>{(feedback?.volumeAdded || completedWorkout.volume).toLocaleString('sv-SE')} kg volym registrerad</li><li><Check size={17}/>Recovery updated</li><li><Check size={17}/>Coach updated</li></ul><button type="button" className="p4-secondary" onClick={onDismiss} aria-label="Stäng workout summary">Klar</button></section>}<section className="panel"><div className="history-toolbar"><SectionTitle eyebrow="Alla pass" title="Träningshistorik"/><div className="search-box small"><Search size={17}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Filtrera historik" aria-label="Filtrera träningshistorik"/></div></div><div className="history-table"><div className="history-row head"><span>Datum</span><span>Pass</span><span>Gym</span><span>Set</span><span>Volym</span><span>Tid</span></div>{filtered.map(h=><div className="history-row" key={h.id}><span>{h.date}</span><strong>{h.name}</strong><span>{h.gym||'—'}</span><span>{h.sets}</span><span>{h.volume.toLocaleString('sv-SE')} kg</span><span>{h.duration} min</span></div>)}</div></section></div>
}

const trendText={improving:'Improving',stable:'Stable',declining:'Declining'}
function ProgressView({history,completedWorkout}){
  const intelligence=useMemo(()=>getProgressIntelligence(history,null,exerciseBank,{}, {recoveryHistory:getAtlasState().recovery?.history||[]}),[history])
  const weekly=intelligence.history.weekly;const monthly=intelligence.history.monthly
  const maxVolume=Math.max(1,...weekly.map(item=>item.volume));const recentPrs=history.flatMap(item=>item.feedback?.newPr||[]).slice(0,5)
  const summary=intelligence.trends.strength.direction==='improving'?'Styrkan utvecklas. Fortsätt med samma progression.':intelligence.trends.volume.direction==='declining'?'Volymen sjunker. Prioritera ett genomförbart pass denna vecka.':'Träningen är stabil. Nästa tydliga steg är konsekvent belastning.'
  return <div className="progress-dashboard" aria-label="Progress dashboard">
    <section className="panel progress-hero"><div><span className="eyebrow">Progress intelligence</span><h2>Din utveckling, tydligt.</h2><p>Deterministiskt beräknad från {history.length} genomförda pass.</p></div><div className="progress-dominant"><strong>{trendText[intelligence.trends.volume.direction]}</strong><span>Volume trend · {intelligence.trends.volume.confidence}% confidence</span></div></section>
    <ProgressSection title="Today's Improvements"><div className="progress-list">{completedWorkout?.feedback?.newPr?.length?completedWorkout.feedback.newPr.slice(0,3).map(pr=><ProgressRow key={`${pr.exerciseId}-${pr.type}`} label={pr.exerciseName} value={`NEW PR · ${pr.type}`}/>):<p className="progress-empty">Complete a workout to reveal today's improvements.</p>}</div></ProgressSection>
    <ProgressSection title="Recent PRs"><div className="progress-list">{recentPrs.length?recentPrs.map((pr,index)=><ProgressRow key={`${pr.exerciseId}-${pr.type}-${index}`} label={pr.exerciseName} value={`NEW PR · ${pr.value}`}/>):<p className="progress-empty">No comparable personal records yet.</p>}</div></ProgressSection>
    <ProgressSection title="Weekly Trend"><VolumeBars data={weekly.slice(-8)} max={maxVolume}/></ProgressSection>
    <ProgressSection title="Monthly Trend"><div className="progress-metrics"><ProgressMetric label="Best month" value={intelligence.records.monthlyVolumeRecord?.period||'—'}/><ProgressMetric label="Record volume" value={intelligence.records.monthlyVolumeRecord?`${intelligence.records.monthlyVolumeRecord.volume.toLocaleString('sv-SE')} kg`:'—'}/></div></ProgressSection>
    <ProgressSection title="Most Improved Muscles"><p className="progress-empty">Muscle rankings appear after exercise-level history is available.</p></ProgressSection>
    <ProgressSection title="Most Consistent Muscles"><ProgressMetric label="Most trained" value={intelligence.consistency.mostTrainedMuscle||'Not enough data'}/></ProgressSection>
    <ProgressSection title="Strength Trend"><TrendMetric trend={intelligence.trends.strength}/></ProgressSection>
    <ProgressSection title="Recovery Trend"><TrendMetric trend={intelligence.trends.recovery}/></ProgressSection>
    <section className="panel progress-coach"><Sparkles size={20}/><div><span className="eyebrow">Coach Summary</span><strong>{summary}</strong><p>{intelligence.consistency.averageWeeklySessions} pass/vecka · längsta svit {intelligence.consistency.longestStreak} dagar.</p></div></section>
    </div>
}
function ProgressSection({title,children}){return <section className="panel progress-section"><h3>{title}</h3>{children}</section>}
function ProgressRow({label,value}){return <div><span>{label}</span><b className={value.includes('NEW PR')?'pr-badge':''}>{value}</b></div>}
function ProgressMetric({label,value}){return <div><span>{label}</span><strong>{value}</strong></div>}
function TrendMetric({trend}){return <div className="trend-metric"><strong>{trendText[trend.direction]}</strong><span>{trend.changePercentage>0?'+':''}{trend.changePercentage}% · {trend.confidence}% confidence</span></div>}
function VolumeBars({data,max}){return data.length?<div className="progress-bars" aria-label="Weekly volume chart">{data.map(item=><div key={item.period}><i style={{height:`${Math.max(4,item.volume/max*100)}%`}}/><span>{item.period.slice(5)}</span><small>{Math.round(item.volume/1000)}t</small></div>)}</div>:<p className="progress-empty">No weekly history yet.</p>}
