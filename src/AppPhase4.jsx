...
function programMeta(program,history=[]){
  ...
  return {exercises,sets,muscles,equipment,level,goal,duration,sessionLength,completed,phase:completed>duration*program?.days*.75?'Peak':completed>duration*program?.days*.35?'Build':'Base'}
}
function Dashboard({programs,history,startProgram,setPage}){
  ...
  return <div className="p4-grid program-dashboard"><section className="p4-hero span8 program-hero"><div><span className="pill"><Sparkles size={15}/>Aktivt program</span><h2>{active?.name||'Inget aktivt program'}</h2><p>{active?`${meta.goal}. Vecka 3 av ${meta.duration} · fas ${meta.phase}. Nästa steg är tydligt: starta rekommenderat pass när du är redo.`:'Välj ett program i biblioteket för att få en tydlig plan.'}</p><div className="overview-meta"><span><CalendarDays size={14}/>Vecka 3 / {meta.duration}</span><span><Dumbbell size={14}/>{active?.days||0} dagar/vecka</span><span><Clock size={14}/>{meta.sessionLength} min/pass</span><span><Target size={14}/>{meta.muscles.slice(0,3).join(' · ')}</span></div><div className="hero-actions"><button className="p4-primary" disabled={!active} onClick={()=>startProgram(active)}><Play size={18}/>Starta nästa workout</button><button className="p4-secondary" onClick={()=>setPage('programs')}>Visa programdetaljer</button></div></div><div className="program-orbit"><strong>{adherence}%</strong><span>adherence</span></div></section>
    <section className="panel span4 next-action-panel"><SectionTitle eyebrow="Nästa åtgärd" title={active?.name||'Välj program'}/><p>{workoutIntelligence.notes}</p><button className="p4-primary full" disabled={!active} onClick={()=>startProgram(active)}><Play size={17}/>Starta nu</button><button className="p4-secondary full" onClick={()=>setPage('programs')}>Byt eller redigera</button></section>
    <Kpi icon={CalendarDays} label="Nuvarande vecka" value={`3 / ${meta.duration}`}/><Kpi icon={Check} label="Veckans pass" value={`${completedThisWeek} / ${active?.days||0}`}/><Kpi icon={Activity} label="Fasprogress" value={`${Math.min(100,Math.round(meta.completed/Math.max(1,totalSessions)*100))}%`}/><Kpi icon={History} label="Senaste följsamhet" value={`${adherence}%`}/>
    <section className="panel span7"><SectionTitle eyebrow="Veckoschema" title="Klart och kommande"/><div className="program-week-grid">{Array.from({length:active?.days||0},(_,i)=>{const done=i<completedThisWeek;const ex=meta.exercises[i%meta.exercises.length];return <article key={i} className={done?'done':i===completedThisWeek?'today':''}><b>{done?<Check size={17}/>:i+1}</b><span><strong>{i%2?'Underkropp / Pull':'Överkropp / Push'}</strong><small>{done?'Genomfört':i===completedThisWeek?'Nästa rekommenderade pass':'Kommande'} · {ex?.name}</small></span></article>})}</div></section>
    <section className="panel span5"><SectionTitle eyebrow="Programstatus" title="Progress och detaljer"/><ProgramProgress meta={meta} program={active}/></section>
    <section className="panel span7"><SectionTitle eyebrow="Senaste aktivitet" title="Recent adherence"/><div className="recent-workouts">{history.slice(0,4).map(item=><div key={item.id}><History size={18}/><span><strong>{item.name}</strong><small>{item.date} · {item.sets} set · {item.duration} min</small></span><b>{item.volume.toLocaleString('sv-SE')} kg</b></div>)}</div></section>
...
function ProgramProgress({program,meta}){return <div className="program-progress"><div className="progress-track"><i style={{width:`${Math.min(100,Math.round(meta.completed/Math.max(1,meta.duration*(program?.days||1))*100))}%`}}/></div><div className="program-facts"><div><span>Duration</span><strong>{meta.duration} veckor</strong></div><div><span>Frequency</span><strong>{program?.days} dagar/vecka</strong></div><div><span>Level</span><strong>{meta.level}</strong></div><div><span>Equipment</span><strong>{meta.equipment}</strong></div></div><p>Fokus: {meta.muscles.join(' · ')}. Återhämtning: 24–48 timmar mellan liknande muskelgrupper.</p></div>}
function ProgramLibrary({programs,setPrograms,activeProgramId,setActiveProgramId,startProgram,notify}){
  const [filter,setFilter]=useState('alla');const [query,setQuery]=useState('');const [sort,setSort]=useState('recommended');const active=programs.find(p=>p.id===activeProgramId)||programs[0]
  const filters=[['alla','Alla'],['favorit','Favoriter'],['Upper/Lower','Upper/Lower'],['PPL','Push/Pull/Legs'],['Helkropp','Full body'],['arkiv','Arkiv']]
  let visible=programs.filter(p=>filter==='arkiv'?p.archived:!p.archived&&(filter==='favorit'?p.favorite:filter==='alla'?true:p.type===filter)).filter(p=>(p.name+p.type+programMeta(p).goal+programMeta(p).level).toLowerCase().includes(query.toLowerCase()))
  visible=[...visible].sort((a,b)=>sort==='days'?a.days-b.days:sort==='duration'?programMeta(a).duration-programMeta(b).duration:Number(b.favorite)-Number(a.favorite))
  const update=(id,patch)=>setPrograms(ps=>ps.map(p=>p.id===id?{...p,...patch}:p));const move=(index,dir)=>{const next=[...active.exercises];const target=index+dir;if(target<0||target>=next.length)return;[next[index],next[target]]=[next[target],next[index]];update(active.id,{exercises:next})};const removeExercise=id=>confirm('Ta bort övningen från programmet?')&&update(active.id,{exercises:active.exercises.filter(x=>x!==id)});const addExercise=id=>{if(!active.exercises.includes(id))update(active.id,{exercises:[...active.exercises,id]})};const meta=programMeta(active)
  return <div className="program-studio"><section className="panel span12 program-library-hero"><div><span className="pill"><Library size={15}/>Program library</span><h2>Välj en plan du faktiskt följer.</h2><p>Sök, filtrera och jämför befintliga program utan ny logik eller påhittad data.</p></div><div className="search-box"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Sök program, mål, nivå eller utrustning"/></div></section><section className="panel program-list"><div className="toolbar"><div className="segmented">{filters.map(([id,l])=><button key={id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{l}</button>)}</div><select aria-label="Sortera program" value={sort} onChange={e=>setSort(e.target.value)}><option value="recommended">Rekommenderat</option><option value="days">Dagar/vecka</option><option value="duration">Längd</option></select></div><div className="program-cards rich">{visible.length?visible.map(p=><ProgramCard key={p.id} program={p} selected={activeProgramId===p.id} onSelect={()=>setActiveProgramId(p.id)}/>):<div className="empty-workout-state"><AlertCircle size={18}/>Inga program matchar sökningen.</div>}</div></section>
    {active&&<section className="panel program-editor program-detail"><div className="editor-head"><div><span className="eyebrow">Program detail</span><input aria-label="Programnamn" value={active.name} onChange={e=>update(active.id,{name:e.target.value})}/><p>{meta.goal} · {meta.level} · {active.days} dagar/vecka</p></div><div><button className="p4-icon" aria-label="Favorit" onClick={()=>update(active.id,{favorite:!active.favorite})}><Star size={18} fill={active.favorite?'currentColor':'none'}/></button><button className="p4-icon" aria-label="Arkivera" onClick={()=>update(active.id,{archived:!active.archived})}><Archive size={18}/></button><button className="p4-primary" onClick={()=>startProgram(active)}><Play size={17}/>Aktivera / starta</button></div></div><ProgramProgress program={active} meta={meta}/><div className="program-detail-tabs"><section><SectionTitle eyebrow="Overview" title="Programöversikt"/><p>{active.name} är ett {active.type}-upplägg för {meta.goal.toLowerCase()} med {meta.sets} arbetsset per runda.</p></section><section><SectionTitle eyebrow="Schedule" title="Weekly structure"/><div className="program-week-grid compact">{Array.from({length:active.days},(_,i)=><article key={i}><b>{i+1}</b><span><strong>Dag {i+1}</strong><small>{i%2?'Pull / Lower focus':'Push / Upper focus'} · 24–48 h återhämtning</small></span></article>)}</div></section><section><SectionTitle eyebrow="Workout days" title="Övningar, set och reps"/><div className="editor-list">{active.exercises.map((id,i)=>{const ex=exerciseBank.find(x=>x.id===id);return <div key={id}><GripVertical size={18}/><span><strong>{i+1}. {ex.name}</strong><small>{ex.muscle} · {ex.equipment} · {ex.sets} · vila 90 sek</small></span><div className="move-buttons"><button aria-label="Flytta upp" onClick={()=>move(i,-1)}><ArrowUp size={15}/></button><button aria-label="Flytta ned" onClick={()=>move(i,1)}><ArrowDown size={15}/></button><button aria-label="Ta bort" onClick={()=>removeExercise(id)}><Trash2 size={15}/></button></div></div>})}</div></section><section><SectionTitle eyebrow="Progression" title="Coaching guidance"/><p>Behåll teknik, öka belastning först när målreps klaras med kontrollerad RPE. Lägg anteckningar i passet före ändring av programlogik.</p></section></div><details className="add-exercise"><summary><Plus size={17}/>Lägg till övning</summary><div>{exerciseBank.filter(e=>!active.exercises.includes(e.id)).map(e=><button key={e.id} onClick={()=>{addExercise(e.id);notify(`${e.name} tillagd`)}}><span><strong>{e.name}</strong><small>{e.muscle} · {e.equipment} · {e.sets}</small></span><Plus size={16}/></button>)}</div></details></section>}</div>
}
function ProgramCard({program,selected,onSelect}){const meta=programMeta(program);return <button className={selected?'selected':''} onClick={onSelect}><span className="program-icon"><Dumbbell size={20}/></span><span><strong>{program.name}</strong><small>{meta.goal} · {meta.level}</small><em>{program.days} dagar/v · {meta.sessionLength} min · {meta.duration} veckor · {meta.equipment}</em></span><span className="program-status">{program.archived?'Arkiv':program.favorite?'Favorit':'Aktiv'}</span><ChevronRight size={17}/></button>}
