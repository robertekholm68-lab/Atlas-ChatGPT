...
export function BottomNavigation({ items, active, onChange }) {
  return <nav className="atlas-bottom-nav" aria-label="Primär navigation">{items.map(({ id, label, icon: Icon }) => <button key={id} type="button" aria-current={active === id ? 'page' : undefined} className={active === id ? 'active' : ''} onClick={() => onChange(id)}><Icon size={20} strokeWidth={2}/><span>{label}</span></button>)}</nav>
}
