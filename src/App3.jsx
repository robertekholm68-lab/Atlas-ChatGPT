...
const pageMeta = {
  ...
  coach:['ASKR Coach','Personliga råd grundade i din träningsdata.']
}
...
function App3(){
  ...
  return <div className="app-shell">
    <aside className="sidebar" aria-label="Huvudnavigation">
      ...
      <div className="sidebar-footer"><button className="profile-card" onClick={()=>setMenuOpen(!menuOpen)}><span className="avatar">RE</span><span><strong>Robert</strong><small>ASKR Member</small></span><Settings size={17}/></button></div>
    </aside>
...
function ExerciseLogger({exercise,exIndex,updateSet,completeSet}){
  ...
  return <section className={`panel exercise-logger ${done===exercise.sets.length?'complete':''}`}>
    ...
    {open&&<div className="set-table"><div className="set-row set-head"><span>Set</span><span>Kg</span><span>Reps</span><span>RPE</span><span>Klar</span></div>{exercise.sets.map((set,i)=><div className={`set-row ${set.done?'done':''}`} key={set.id}><span>{i+1}</span><NumberStepper value={set.kg} step={2.5} onChange={v=>updateSet(exIndex,i,{kg:v})}/><NumberStepper value={set.reps} step={1} onChange={v=>updateSet(exIndex,i,{reps:v})}/><select value={set.rpe} onChange={e=>updateSet(exIndex,i,{rpe:Number(e.target.value)})}>{[6,7,8,9,10].map(v=><option key={v}>{v}</option>)}</select><button className="set-check" onClick={()=>completeSet(exIndex,i,exercise.rest)}>{set.done?<Check size={18}/>:<span/>}</button></div>)}</div>}
  </section>
}
