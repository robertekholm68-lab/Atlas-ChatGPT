...
## Data sources
The orchestration layer consumes local ATLAS state without mutating it:
...
5. The primary action is the smallest useful next step and routes only to an existing ATLAS screen.
...
When workout and recovery data are missing, ATLAS does not fabricate readiness, goals, or predictions. The recommendation becomes an honest setup state that asks the user to log daily form and workouts.

## Event integration
Relevant ATLAS events regenerate insights and the coach recommendation after state updates: workout completion, profile update, goal update, recovery update, body measurement update, prediction regeneration, insight regeneration, and daily check-in. Unrelated UI navigation does not dispatch those events.
...
## Limitations
The engine is local and deterministic. It does not diagnose medical issues, call external APIs, or replace professional advice. It can only use data already present in ATLAS state.
